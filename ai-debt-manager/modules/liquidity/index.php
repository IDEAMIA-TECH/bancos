...
$avg_daily_income = !empty($daily_income) ? array_sum($daily_income) / count($daily_income) : 0;
$avg_daily_expenses = !empty($daily_expenses) ? array_sum($daily_expenses) / count($daily_expenses) : 0;
...
$current_balance = !empty($daily_balances) ? end($daily_balances) : 0;
...
$min_balance = !empty($predictions) ? min($predictions) : 0;
$risk_level = 'low';
if ($min_balance < 0) {
    $risk_level = 'high';
    ...
    <div class="container">
        ...
        <?php if (!$has_transactions): ?>
            <div class="alert alert-info">
                ...
                No hay transacciones disponibles para los últimos 3 meses. 
                Conecta tus cuentas bancarias para ver tus predicciones de liquidez.
            </div>
        <?php else: ?>
